...
class TestTree:
    ...
    def test_count_segregating_snps(self):
        ...
        tree = Tree.initialized(
            src_genomes[0], rec_genomes[0], source_pop, recipient_pop
        )
        shared_src_segs = tree.count_segregating_snps(
            "source", tree.shared_branch
        )
        assert shared_src_segs == 2, "may fail due to random sampling"  # [3,4]
        shared_rec_segs = tree.count_segregating_snps(
            "recipient", tree.shared_branch
        )
    ...
    def test_standard_entropy(self):
        ...
        assert tree.standard_entropy([1, 1, 1]) > tree.standard_entropy(
            [1, 0, 2]
        )
    ...
    def test_check_clumpiness(self):
        ...
        branch = {1: {"source_proportion": 1, "recipient_proportion": 1}}
        ...
        branch = {
            1: {"source_proportion": 1, "recipient_proportion": 1},
            2: {"source_proportion": 0.5, "recipient_proportion": 1},
        }
    ...
    def test_check_clumpiness_composite(self):
        ...
        assert tree.check_clumpiness_composite(num_bins=10) == {
            "ancestral to source lineage": {"source": 0, "recipient": 0},
            "ancestral to recipient lineage": {"source": 0, "recipient": 0},
        }

        tree.shared_branch = {
            1: {"source_proportion": 0.8, "recipient_proportion": 1},
            2: {"source_proportion": 0.5, "recipient_proportion": 1},
        }
        tree.source_branch = {
            3: {"source_proportion": 1, "recipient_proportion": 0},
            4: {"source_proportion": 0.5, "recipient_proportion": 0},
        }
        tree.recipient_branch = {
            5: {"source_proportion": 0, "recipient_proportion": 1},
            6: {"source_proportion": 0.5, "recipient_proportion": 0.8},
        }
        output = tree.check_clumpiness_composite(num_bins=10)
        assert (
            output["ancestral to source lineage"]["source"]
            > output["ancestral to source lineage"]["recipient"]
        )
        assert (
            output["ancestral to recipient lineage"]["source"]
            > output["ancestral to recipient lineage"]["recipient"]
        )

        tree.shared_branch = {
            1: {"source_proportion": 1, "recipient_proportion": 1},
            2: {"source_proportion": 0.7, "recipient_proportion": 0.8},
        }
        tree.source_branch = {
            3: {"source_proportion": 1, "recipient_proportion": 0},
        }
        tree.recipient_branch = {
            4: {"source_proportion": 0, "recipient_proportion": 1},
        }
        output = tree.check_clumpiness_composite(num_bins=10)
        assert (
            output["ancestral to source lineage"]["source"]
        ...
        tree.source_branch = {
            3: {"source_proportion": 0.3, "recipient_proportion": 0},
            4: {"source_proportion": 0.5, "recipient_proportion": 0},
        }
        tree.recipient_branch = {
            5: {"source_proportion": 0, "recipient_proportion": 0.3},
            6: {"source_proportion": 0, "recipient_proportion": 0.5},
        }
        output = tree.check_clumpiness_composite(num_bins=10)
        assert (
            output["ancestral to source lineage"]["source"]
